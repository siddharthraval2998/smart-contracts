...
contract BanditLootGold is ERC20 { // is  - extends
    ...
    constructor(address creator) ERC20("BanditLootGold","BLG"){ //name and symbol for every erc20
            owner = msg.sender;
            _mint(creator, 1000 * 1e18);
    }
    //airdropping - give free tokens to few people to kickstart the flywheel
...
contract BanditLoot is ERC721URIStorage {
    ...
    function fight(uint256 tokenId, uint256 accuracyBoosterInBanditGold) public returns(string memory){
        ...
        if(banditScore > playerScore){
            //return string(abi.encodePacked("Bandit #",banditLootId," won by ", banditScore - playerScore, "points. Your average accuracy was ",totalAccuracy/5)); //deprecated
